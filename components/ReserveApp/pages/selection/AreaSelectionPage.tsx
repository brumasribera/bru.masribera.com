...
export function AreaSelectionPage({ project, onBack, onContinue }: AreaSelectionPageProps) {
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-green-50 to-emerald-100 overflow-hidden relative">
      {/* Full width map layout - removed grid constraints */}
      <div className="relative w-full h-full">
        ...
        {/* Confirmation Button - bottom center (all screen sizes) */}
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2">
          {totalSelectedArea > 0 ? (
            <button
              onClick={handleConfirmClick}
              className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-full font-medium shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 border border-green-500 hover:border-green-600 flex items-center space-x-2 whitespace-nowrap"
            >
              <span className="text-sm font-semibold">Protect {totalSelectedArea} m²</span>
              <span className="text-xs opacity-90 ml-1">
                €{(totalSelectedArea * project.pricePerM2).toFixed(2)}
              </span>
            </button>
          ) : (
            <div className="bg-gray-400 text-white px-6 py-3 rounded-full font-medium shadow-lg border border-gray-300 flex items-center space-x-2 opacity-75 whitespace-nowrap">
              <span className="text-sm font-semibold">Select area to continue</span>
            </div>
          )}
        </div>
      </div>
